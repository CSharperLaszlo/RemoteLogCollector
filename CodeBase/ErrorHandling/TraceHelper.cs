...
using RemoteConnectionToLinux;

namespace CodeBase.ErrorHandling
{
    public class TraceHelper : IDisposable
    {
            const string FILE_LISTENER_NAME = "RemoteHelperListener";
        ...
        static public void MarkTraceFileForStore(string reason_in)
        {
            ...
            {
                case "":
                    reason_in = "No reason given";
                    break;
                default:
                    if (reason_in.Length > 50)
                        reason_in = reason_in.Substring(0, 50); //limit the length of the reason    
                    break;
            }
                
            Trace.TraceInformation(MainWindow.DetailedTraceInfo);
            ...
            void DeleteOldFilesThread()
            {
                ...
                Trace.TraceInformation("DeleteOldFiles finished, deleted files:");
                for (int i = 0; i < query.ToList<string>().Count; i++)
                {
                    Trace.TraceError(string.Format("Deleted File: {0}.", query.ElementAtOrDefault<string>(i)));
                }
                
                }
                catch (Exception e)
                {
                    Trace.TraceWarning("Exception at DeteteOldFilesThread:{0}", string.Format("Exception messsage: \r\n{0}\r\n{0}",e.Message,e.InnerException));
                }
            }
            #endregion
